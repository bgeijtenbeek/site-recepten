...
describe('recipe discovery', () => {
  ...
  it('filters on one selected moment', () => {
    expect(filterRecipes(records, '', 'Overig', []).map(({ id }) => id)).toEqual(['groenten']);
  });
  ...
  it('combines query, moment, and selected kenmerken with AND behavior', () => {
    expect(filterRecipes(records, 'kip', 'Hoofdgerechten', ['Rijst']).map(({ id }) => id)).toEqual(['kip-rijst']);
    expect(filterRecipes(records, 'kip', 'Overig', ['Rijst'])).toEqual([]);
    expect(filterRecipes(records, 'kip', 'Hoofdgerechten', ['Vega'])).toEqual([]);
  });
  ...
  it('treats blank search as inactive', () => {
    ...
    expect(isDiscoveryActive('', 'Overig', [])).toBe(true);
